...
class OptractNode extends PubSubNode {
	constructor(cfgObj) {
		...
		// IPFS related
		this.ipfs = new ipfsClient('optract-ipfs-gateway.default', '5001', {protocol: 'http'}); //FIXME: need to setup onReady event for IPFS

		// DEBUG
		this.ipfsId = () => { return this.ipfs.id() };
		this.p2pStats = () => { return this.stats() };
...
let stage = Promise.resolve(opt).catch((err) => { process.exit(1); }).then(() => 
{
	r = new WSServer({ port: 59437, host: '0.0.0.0' });

	const expose = 
	{
		vars: ['networkID', 'userWallet'],
		stat: ['ipfsId', 'getPrevBlockData', 'p2pStats', 'getBlockNo', 'ethNetStatus'],
		func: ['getOproundInfo', 'memberStatus', 'getOproundLottery', 'getBlockData', 'get', 'isValidator'],
